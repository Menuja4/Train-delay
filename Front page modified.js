...
for(let i = 0; i < NP; i++) {
    particles.push({
        ...
        color: Math.random() > 0.5 ? 
            'rgba(111,241,255,' + (0.06 + Math.random() * 0.35) + ')' : 
            'rgba(160,75,255,' + (0.06 + Math.random() * 0.35) + ')'
    });
}
...
function draw() {
    ...
    for(let i = 0; i < 8; i++) {
        ctx.beginPath();
        let gx = (i % 2 === 0 ? window.innerWidth * 0.14 : window.innerWidth * 0.86) + 
                 Math.sin(Date.now() / 4000 + i) * 60 * (i % 2 ? -1 : 1);
        let gy = window.innerHeight * (0.18 + (i * 0.07));
        let rad = 260 + i * 60;
        let col = (i % 3 === 0) ? 'rgba(111,241,255,0.06)' : 
                 (i % 3 === 1) ? 'rgba(160,75,255,0.06)' : 
                 'rgba(255,139,75,0.05)';
        ctx.fillStyle = col;
